...
const SignupForm: React.FC = () => {
  ...
  const handleConfirmDays = () => {
    ...
    const updatedSchedules = schedules.map((schedule, index) =>
      index === currentScheduleIndex ? { ...schedule, days: selectedDays } : schedule
    );
  ...
  const handleTimeChange = (index: number, timeIndex: number, newValue: Date | null) => {
      ...
      <form onSubmit={handleSubmit}>
        ...
        {currentStep === 1 && (
          <Box>
            ...
            />
            <Button variant="outlined" onClick={handleConfirmFrequency} fullWidth sx={{ mt: 2 }}>
              확인
            </Button>
          </Box>
        )}
        ...
        {currentStep === 2 && (
          <Box>
            ...
            <ButtonGroup variant="contained" aria-label="text button group" fullWidth>
                {daysOfWeek.map(day => (
                <Button
                    key={day}
                    onClick={() => handleDayChange(day)}
                    color={selectedDays.includes(day) ? 'primary' : 'secondary'}
                    disabled={selectedDays.length >= tempFrequency && !selectedDays.includes(day)}
                >
                    {day}
                </Button>
                ))}
            </ButtonGroup>
            <Button variant="outlined" onClick={handleConfirmDays} fullWidth sx={{ mt: 2 }}>
              확인
            </Button>
          </Box>
        )}
                ...
                <Grid item xs={12} md={6} key={timeIndex}>
                  <LocalizationProvider dateAdapter={AdapterDateFns}>
                    <TimePicker
                      ...
                      value={time}
                      onChange={(newValue) => handleTimeChange(currentScheduleIndex, timeIndex, newValue)}
                    />
                  </LocalizationProvider>
                </Grid>
              ))}
              <Grid item xs={12}>
                <Button variant="outlined" onClick={handleConfirmTimes} fullWidth sx={{ mt: 2 }}>
                  스케줄 추가
                </Button>
              </Grid>
            </Grid>
            ...
            <Box key={schedule.schedule_id} sx={{ mb: 3 }}>
              <Grid container spacing={2} alignItems="center">
                ...
                {schedule.times.map((time, timeIndex) => (
                  <Grid item xs={12} md={6} key={timeIndex}>
                    <Typography>시간 {timeIndex + 1}: {time ? time.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' }) : '미설정'}</Typography>
                  </Grid>
                ))}
                <Grid item xs={12}>
                  <IconButton onClick={() => handleRemoveSchedule(schedule.schedule_id)} color="error">
                    <DeleteIcon />
                  </IconButton>
                </Grid>
              </Grid>
